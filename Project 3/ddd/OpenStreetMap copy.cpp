...
struct COpenStreetMap::SImplementation
{
    ...
    vector<SWay> allWays;
    SImplementation(shared_ptr<CXMLReader> reader)
    {
        ParseOSM(reader);
    }

    void ParseOSM(shared_ptr<CXMLReader> &reader)
    {
        ...
        while (!reader->End())
        {
            ...
            if (!reader->ReadEntity(entity, false))
            {
                break;
            }
            ...
            if (entity.DType == SXMLEntity::EType::StartElement)
            {
                ...
                else if (entity.DNameData == "tag")
                {
                    ...
                    {
                        currNode.attributes.push_back(make_pair(key, value));
                    }
                    ...
                    {
                        currWay.attributes.push_back(make_pair(key, value));
                    }
                }
...
COpenStreetMap::COpenStreetMap(shared_ptr<CXMLReader> src) : DImplementation(make_unique<SImplementation>(src)){}
