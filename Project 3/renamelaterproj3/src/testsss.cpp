...
// We will be using XMLReader, Writer, Entity to read the OSM file.
// The COpenStreetMap class you will be developing will implement the CStreetMap
// abstract interface. There are two struct abstract interfaces that are part of the CStreetMap, that
// will need to be implemented as well (SNode and SWay). The street map is a collection of nodes,
// and ways. Each way is a set of nodes that specifies a street path. The nodes and ways may have
// attributes associated with them that appear in tag elements. For example, a way might specify if
// bikes are allowed on the way or not.
